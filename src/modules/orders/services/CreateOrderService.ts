...
class CreateOrderService {
  ...
  public async execute({ customer_id, products }: IRequest): Promise<Order> {
    ...
    const findProductsWithNoQuantityAvailable = products.filter(product => {
      const response = existentProducts.filter(p => {
        if (p.quantity <= product.quantity && p.id === product.id) {
          return p;
        }
        return undefined;
      });
    ...
    if (findProductsWithNoQuantityAvailable.length) {
      throw new AppError(
        `Product ${findProductsWithNoQuantityAvailable[0].id} has no quantity available to attend ${findProductsWithNoQuantityAvailable[0].quantity}`,
      );
    }
